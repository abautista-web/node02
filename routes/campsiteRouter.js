...
campsiteRouter
  ...
  //Week 3 Task 2 added authenticate.verifyAdmin
...
campsiteRouter
  ...
  .put(authenticate.verifyUser, (req, res, next) => {
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
 // Week3 Task 4 added outer if/else line 206/222-226                                                    
          if (req.user._id.equals(campsite.comments.id(req.params.commentId).author._id)) {
            ...
            err = new Error("You can not update comments because your are not the author");
            err.status = 403;
            return next(err);
          }
 //End Week3 Task 4 added outer if/else 
        } else if (!campsite) {
          err = new Error(`Campsite ${req.params.campsiteId} not found`);
          err.status = 404;
          return next(err);
  ...
  .delete(authenticate.verifyUser, (req, res, next) => {
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
 // Week3 Task 4 added outer if/else ln 245/255-259
          if (req.user._id.equals(campsite.comments.id(req.params.commentId).author._id)) {
